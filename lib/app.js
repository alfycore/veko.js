...
<body>
  ...
  /**
   * Route to a PHP-like template file
   * Usage: app.phpRoute('/about', 'about.php', { title: 'About' })
   */
  phpRoute(routePath, template, options = {}) {
    if (!this.isVSVEnabled()) {
      throw new Error('VSV not enabled. Call enableVSV() first.');
    }
    ...
    this.get(routePath, async (req, res) => {
      ...
      if (options.getData) {
        const fetched = await options.getData(req, res);
        Object.assign(data, fetched);
      }

      try {
        const html = await this.vsv.php.renderFile(template, data, req, res);
        if (!res.writableEnded) {
          res.html(html);
        }
      } catch (e) {
        if (e.__redirect) return; // redirect was handled
        throw e;
      }
    });
   ...
   */
  async renderPHP(template, data = {}, req = null, res = null) {
    if (!this.isVSVEnabled()) {
      throw new Error('VSV not enabled. Call enableVSV() first.');
    }
    return this.vsv.php.renderString(template, data, req, res);
  }

  /**
   * Register a global variable for PHP templates
   */
  phpGlobal(name, value) {
    if (!this.isVSVEnabled()) {
      throw new Error('VSV not enabled. Call enableVSV() first.');
    }
    this.vsv.php.setGlobal(name, value);
    return this;
  }

  /**
   * Register a custom PHP-like function
   */
  phpFunction(name, fn) {
    if (!this.isVSVEnabled()) {
      throw new Error('VSV not enabled. Call enableVSV() first.');
    }
    this.vsv.php.registerFunction(name, fn);
    return this;
  }

  // Listen
  listen(port, callback) {
    // Support listen(callback) without port
    if (typeof port === 'function') {
      callback = port;
      port = null;
    }
    const finalPort = Number(port) || this.options.port;

...
class VSV {
  ...
  registerMiddleware() {
    ...
    this.app.express.get('/_vsv/runtime.js', (req, res) => {
      res.type('application/javascript');
      res.send(this.runtime.getClientRuntime());
    });
    
    // Serve compiled components
    this.app.express.get('/_vsv/components/:name.js', (req, res) => {
      const compiled = this.compiledCache.get(req.params.name);
      if (compiled) {
        res.type('application/javascript');
        res.send(compiled.client);
      } else {

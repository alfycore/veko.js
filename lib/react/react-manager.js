...
class ReactManager {
  ...
  async loadReact() {
    try {
      // Try to load from user's project first
      ...
      // Check if React exists in project
      if (fs.existsSync(reactPath) && fs.existsSync(reactDomPath)) {
        this.React = require(reactPath);
        this.ReactDOM = require(reactDomPath);
        this.ReactDOMServer = require(path.join(reactDomPath, 'server'));
      } else {
        // Fallback to normal require (if veko has react as dependency)
        this.React = require('react');
        this.ReactDOM = require('react-dom');
        this.ReactDOMServer = require('react-dom/server');
      }
    } catch (error) {
      throw new Error(`Cannot load React: ${error.message}. Run: npm install react react-dom`);
    }
  }
  ...
  async compileComponent(component) {
    ...
    if (!component || !component.path) {
      throw new Error(`Invalid component: path is undefined for ${component?.name || 'unknown'}`);
    }
  ...
  async compileForServer(component) {
    ...
    if (!fs.existsSync(component.path)) {
      throw new Error(`Component file not found: ${component.path}`);
    }

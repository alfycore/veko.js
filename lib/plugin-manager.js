...
async loadAllPlugins() {
  ...
  // Collect plugin dependencies first
  const pluginsInfo = [];
  for (const file of pluginFiles) {
    try {
      const pluginName = file.replace('.js', '');
      const pluginPath = path.join(process.cwd(), this.options.pluginsDir, file);
      ...
      // Debug log to see the dependencies
  ...
  // Debug log to see the calculated load order
  ...
  for (const pluginName of loadOrder) {
    try {
      ...
      this.log('error', `Échec du chargement`, `${pluginName} → ${error.message}`);
    }
  }
...
sortPluginsByDependencies(pluginsInfo) {
  ...
  // Helper function to load a plugin and its dependencies
  const loadPlugin = (plugin) => {
    ...
    for (const dep of plugin.dependencies) {
      const depPlugin = pluginsInfo.find(p => p.name === dep);
      if (depPlugin) {

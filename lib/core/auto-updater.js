        ...
        try {
            ...
            if (stats.size > 1024 * 1024) { // 1MB
                ...
                logFiles.forEach(file => {
                    fs.unlinkSync(path.join(path.dirname(this.logPath), file));
                });
            }
        } catch (error) {
        ...
        try {
            ...
            if (updateInfo.hasUpdate) {
                ...
                if (autoUpdate.toLowerCase().startsWith('o')) {
                    ...
                    // Lance la mise à jour immédiatement
                    await this.performUpdate(updateInfo);
                } else {
                    console.log(this.styles.info('ℹ️ Vous pouvez activer les mises à jour automatiques plus tard avec: npx veko-update config'));
                }
            } else if (updateInfo.needsInstall) {
                console.log(this.styles.warning('⚠️ Veko n\'est pas encore installé correctement.'));
            } else {
            ...
            if (this.config.autoCheck) {
                console.log(this.styles.info('⏰ Vérifications automatiques activées (toutes les heures)'));
            }
...
║                  🎉 BIENVENUE DANS VEKO!                 ║
║                                                           ║
║  L'auto-updater le plus avancé de l'écosystème Node.js   ║
║                                                           ║
║  ✨ Mises à jour automatiques                            ║
║  🔒 Backups et rollbacks sécurisés                       ║
...
╚═══════════════════════════════════════════════════════════╝
        ...
        const quickSetup = await this.askQuestion(
            this.styles.accent('Voulez-vous utiliser la configuration rapide ? (o/n)'),
            'o'
        );
        
        if (quickSetup.toLowerCase().startsWith('o')) {
            // Configuration rapide
            this.config.autoCheck = true;
            this.config.autoUpdate = false;
            this.config.notifications = true;
            this.config.progressBar = true;
            this.config.rollbackOnFailure = true;
            
            console.log(this.styles.success('✅ Configuration rapide appliquée:'));
            console.log(this.styles.info('   • Vérifications automatiques: Activées'));
            console.log(this.styles.info('   • Mises à jour automatiques: Désactivées (sécurité)'));
            console.log(this.styles.info('   • Notifications: Activées'));
            console.log(this.styles.info('   • Rollback automatique: Activé'));
        } else {
    ...
    static async handleCLI(args = []) {
        ...
        switch (command) {
            ...
            default:
                // Si aucune commande, lance le menu interactif
                this.initReadline();
    ...
    static showCLIHelp() {
        ...
        console.log(this.styles.separator);
        console.log(this.styles.info('Usage: npx veko-update [commande]'));
        console.log('');
        console.log(this.styles.menu('Commandes disponibles:'));
        console.log(this.styles.highlight('  activate') + '    🚀 Active l\'auto-updater (premier lancement)');
        console.log(this.styles.highlight('  check') + '       🔍 Vérifie les mises à jour disponibles');
        console.log(this.styles.highlight('  update') + '      ⬆️  Lance la mise à jour si disponible');
        console.log(this.styles.highlight('  config') + '      ⚙️  Ouvre la configuration interactive');
        console.log(this.styles.highlight('  stats') + '       📊 Affiche les statistiques d\'utilisation');
        console.log(this.styles.highlight('  backup') + '      💾 Gère les backups (restaurer/supprimer)');
        console.log(this.styles.highlight('  rollback') + '    🔄 Restaure une version précédente');
        console.log(this.styles.highlight('  daemon') + '      👾 Lance le daemon en arrière-plan');
        console.log(this.styles.highlight('  stop') + '        ⏹️  Arrête le daemon');
        console.log(this.styles.highlight('  status') + '      ℹ️  Affiche le statut de l\'auto-updater');
        console.log(this.styles.highlight('  help') + '        ❓ Affiche cette aide');
        console.log('');
        console.log(this.styles.info('Options:'));
        console.log(this.styles.highlight('  --version, -v') + '   Affiche la version');
        console.log(this.styles.highlight('  --help, -h') + '      Affiche cette aide');
        console.log('');
        console.log(this.styles.dim('Exemples:'));
        console.log(this.styles.dim('  npx veko-update              # Menu interactif'));
        console.log(this.styles.dim('  npx veko-update check        # Vérifier les mises à jour'));
        console.log(this.styles.dim('  npx veko-update update       # Mettre à jour maintenant'));
        console.log(this.styles.dim('  npx veko-update config       # Configurer l\'auto-updater'));
        console.log(this.styles.separator);
    }

    // 🔢 Affichage de la version
    static showVersion() {
        console.log(this.styles.version('Veko Auto-Updater v2.0.0'));
        console.log(this.styles.dim('Le meilleur auto-updater de l\'écosystème Node.js'));
    }

    // 👾 Daemon en arrière-plan
    static async startDaemon() {
        try {
            console.log(this.styles.info('👾 Démarrage du daemon auto-updater...'));
            ...
            console.log(this.styles.success(`✅ Daemon démarré (PID: ${process.pid})`));
            console.log(this.styles.info('🔍 Vérifications automatiques activées'));
            console.log(this.styles.dim('Utilisez Ctrl+C pour arrêter ou "npx veko-update stop"'));
            ...
            process.on('SIGINT', async () => {
                ...
                console.log(this.styles.success('✅ Daemon arrêté'));
                process.exit(0);
            });
            
            // Boucle infinie pour maintenir le daemon
            while (true) {
                ...
                try {
                    ...
                    if (updateInfo.hasUpdate && this.config.autoUpdate) {
                        console.log(this.styles.info('🚀 Mise à jour automatique en cours...'));
                        await this.performUpdate(updateInfo);
                    }
                } catch (error) {
                    this.log('error', `Erreur dans le daemon: ${error.message}`);
                }
            }
            
        } catch (error) {
            this.log('error', `Erreur lors du démarrage du daemon: ${error.message}`);
            console.log(this.styles.error(`❌ Erreur: ${error.message}`));
        }
    }
    ...
    static async stopDaemon() {
        try {
            ...
            if (!fs.existsSync(pidFile)) {
                console.log(this.styles.warning('⚠️ Aucun daemon en cours d\'exécution'));
                return;
            }
            ...
            try {
                ...
                fs.unlinkSync(pidFile);
                console.log(this.styles.success(`✅ Daemon arrêté (PID: ${pid})`));
            } catch (error) {
                if (error.code === 'ESRCH') {
                    console.log(this.styles.warning('⚠️ Le daemon n\'est plus actif'));
                    fs.unlinkSync(pidFile);
                } else {
                    throw error;
                }
            ...
            this.log('error', `Erreur lors de l'arrêt du daemon: ${error.message}`);
            console.log(this.styles.error(`❌ Erreur: ${error.message}`));
        }
    }

    // ℹ️ Statut de l'auto-updater
    static async showStatus() {
        try {
            await this.init();
            
            console.log(this.styles.title('ℹ️ Statut de l\'auto-updater Veko'));
            console.log(this.styles.separator);
            
            // Statut général
            const currentVersion = this.getCurrentVersion();
            console.log(this.styles.info(`📦 Version Veko: ${currentVersion || 'Non installé'}`));
            ...
            if (daemonRunning) {
                const pid = fs.readFileSync(pidFile, 'utf8').trim();
                try {
                    process.kill(parseInt(pid), 0); // Test si le processus existe
                    console.log(this.styles.success(`👾 Daemon: Actif (PID: ${pid})`));
                } catch {
                    console.log(this.styles.warning('👾 Daemon: Inactif (PID obsolète)'));
                    fs.unlinkSync(pidFile);
                }
            } else {
                console.log(this.styles.dim('👾 Daemon: Inactif'));
            }
            
            // Configuration
            console.log(this.styles.info(`⚙️ Vérifications auto: ${this.config.autoCheck ? '✅' : '❌'}`));
            console.log(this.styles.info(`🔄 Mises à jour auto: ${this.config.autoUpdate ? '✅' : '❌'}`));
            ...
            // Statistiques
            console.log(this.styles.info(`📊 Mises à jour totales: ${this.stats.totalUpdates}`));
            console.log(this.styles.info(`🔄 Rollbacks: ${this.stats.rollbacks}`));
            console.log(this.styles.info(`🕐 Dernière vérification: ${this.stats.lastCheck || 'Jamais'}`));
            console.log(this.styles.info(`🕐 Dernière mise à jour: ${this.stats.lastUpdate || 'Jamais'}`));
            
            // Backups
            try {
                const backups = fs.readdirSync(this.backupDir).filter(dir => dir.startsWith('backup-'));
                console.log(this.styles.info(`💾 Backups disponibles: ${backups.length}`));
            } catch {
                console.log(this.styles.info('💾 Backups disponibles: 0'));
            }
            
            console.log(this.styles.separator);
            
        } catch (error) {
            this.log('error', `Erreur lors de l'affichage du statut: ${error.message}`);
            console.log(this.styles.error(`❌ Erreur: ${error.message}`));
        }
    }

    // 🎯 Hook d'installation automatique
    static async postInstallHook() {
        try {
            // Vérifie si c'est la première installation
            const isFirstInstall = !fs.existsSync(this.configPath);
            
            if (isFirstInstall) {
                console.log(this.styles.title('\n🎉 Veko installé avec succès!'));
                console.log(this.styles.info('🚀 Activation de l\'auto-updater...'));
                
                // Active automatiquement l'auto-updater
                setTimeout(async () => {
                    try {
                        this.initReadline();
                        await this.activate();
                        ...
                        console.log(this.styles.warning('⚠️ Activation manuelle requise: npx veko-update activate'));
                    }
                }, 1000);
            } else {
                // Installation existante - vérification rapide
                console.log(this.styles.info('🔍 Vérification de la configuration...'));
                await this.init();
                
                const updateInfo = await this.checkForUpdates(true);
                if (updateInfo.hasUpdate) {
                    console.log(this.styles.warning('💡 Une mise à jour Veko est disponible!'));
                    console.log(this.styles.info('   Lancez: npx veko-update check'));
                }
            }
            
        } catch (error) {
            console.log(this.styles.dim('💡 Lancez "npx veko-update activate" pour configurer l\'auto-updater'));
        }
    }
    ...
    static async main(args = process.argv.slice(2)) {
        try {
            // Si appelé depuis un script npm, active le hook
            if (process.env.npm_lifecycle_event === 'postinstall') {
                await this.postInstallHook();
                return;
            }
            
            // Sinon, traite les arguments CLI
            await this.handleCLI(args);
            
        } catch (error) {
            console.log(this.styles.error(`❌ Erreur fatale: ${error.message}`));
            process.exit(1);
        }
    }
}
